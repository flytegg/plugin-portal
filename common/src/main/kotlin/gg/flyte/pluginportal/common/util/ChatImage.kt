...
object ChatImage {
    ...
    class ImageTextBuilder(imageURL: String) {
        ...
        private val originalLines = getImageLines(imageURL).map { translate(it) }
...
fun getAverageColor(image: BufferedImage): Color {
    val (sumR, sumG, sumB) = image.getRGB(0, 0, image.width, image.height, null, 0, image.width).asSequence()
        .map { Color(it) }.fold(Triple(0, 0, 0)) { (r, g, b), color ->
...
fun Plugin.buildMoreTooltip(): Component {
    var tooltip = textPrimary("Platform Details:")

    platforms.asList().forEach { platform ->
        tooltip = tooltip.append(Component.newline()).append(Component.newline())
            .append(textPrimary("${platform.platform.name}:").bold()).append(Component.newline())
            .append(textSecondary("Author: ")).append(text(platform.author)).append(Component.newline())
            .append(textSecondary("Downloads: ")).append(text(platform.downloads.format()))

        // Add platform-specific data
        when (platform) {
            is ModrinthPlatformEntry -> {
                tooltip = tooltip.append(Component.newline()).append(textSecondary("Followers: "))
                    .append(text(platform.followers.toString()))
            }

            is SpigotMCPlatformEntry -> {
                platform.rating.let { rating ->
                    tooltip = tooltip.append(Component.newline()).append(textSecondary("Rating: "))
                        .append(text("${rating.average}/5.0 (${rating.count} reviews)"))
                }
            }

            is HangarPlatformEntry -> {
                tooltip = tooltip.append(Component.newline()).append(textSecondary("Stars: "))
                    .append(text(platform.stars.toString()))
            }

            is PolymartPlatformEntry -> {
                platform.reviews.let { reviews ->
                    tooltip = tooltip.append(Component.newline()).append(textSecondary("Reviews: "))
                        .append(text("${reviews.stars}/5.0 (${reviews.count} reviews)"))
                }
                
                platform.premium?.let { premium ->
                    tooltip = tooltip.append(Component.newline()).append(textSecondary("Price: ")).append(
                        text(
                            "${premium.price} ${premium.currency}",
                            NamedTextColor.GOLD
                        )
                    ).clickEvent(ClickEvent.openUrl(platform.webpageURL))
                }
            }
        }

        tooltip = tooltip.append(Component.newline()).append(textSecondary("Last Updated: "))
        ...
        platform.newestCompatibleVersion(null, currentServerTypePreference(), currentMinecraftVersion())?.let { version ->
            ...
            tooltip = tooltip.append(if (ServerPlatform.FOLIA in version.serverPlatforms) text("Yes", NamedTextColor.GREEN) else text("No", NamedTextColor.RED))

            tooltip = tooltip.append(Component.newline()).appendSecondary("Supports: ${version.supportLabel}")
        } ?: run {
            tooltip = tooltip.append(Component.newline()).appendSecondary("Latest Version: ").append(text("No compatible version"))
        }
    }
...
fun Plugin.getImageComponent(): Component {
    val description = splitDescriptionIntoLines(sanitisedDescription ?: "", 160, MAX_DESCRIPTION_LINES)
    ...
    val image = ChatImage.ImageTextBuilder(iconURL ?: "").setLine(
            0, centerComponentLine(
                textPrimary(name.shortenToLine(80, true)).bold()
                    .clickEvent(ClickEvent.openUrl(platforms.best?.webpageURL ?: "https://pluginportal.link")), 160
            )
        ).apply {
            description.forEachIndexed { index, line ->
                setLine(index + 2, textSecondary(line).showOnHover(sanitisedDescription ?: ""))
            }
        }.setLine(description.size + 3, textSecondary("Downloads: ${totalDownloads.format()}"))
        .setLine(description.size + 4, textSecondary("Platforms: ${platforms.available.joinToString()}"))
        .setLine(description.size + 5, textSecondary("Versions: $supportedVersions")).setLine(
            11, centerComponentLine(
                text("").let {
                    if (localPlugin != null) it.append(SharedComponents.getUpdateButton(localPlugin)).append(text(" "))
                    else it.append(text("    "))
                }.append(
                    SharedComponents.getInstallButton(this, installed)
                ).append(text("  ")).append(
                    textSecondary("[More]")
//                        .showOnHover(buildMoreTooltip())
                        .hoverEvent(HoverEvent.showText(buildMoreTooltip()))
                ), 160
            )
        ).build()
    return image
